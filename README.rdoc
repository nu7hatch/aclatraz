...
First of all you have to initialize ACL system with specified datastore. Here's
an example with Redis:

  Aclatraz.init :redis, "redis://localhost:6379/0"
  
Then you have to define your suspects, eg.  

  class Account < ActiveRecord::Base
    include Aclatraz::Suspect
  end
  
Now your suspect have few methods which will helps you manage permissions:

  @account = Account.create
  @account.roles.has?(:foo)    # => false
  @account.roles.assign(foo)   
  @account.roles.has?(:foo)    # => true
  @account.roles.delete(foo)   
  
  @account.is.foo?             # => false
  @account.is_not.foo?         # => true
  @account.is.foo!             # shortcut for roles.assign(:foo)
  @account.is_not.foo!         # shortcut for roles.delete(:foo)
...
Last step is create you access control list and set guards:
  ...
  class Foo
    ...
    # The #suspects method allows to pass String, Symbol or Object as suspect. 
